...
const StudentDetailsFakerDialogComponent = (props) => {
    ...
    const [courseNameIds, setcourseNameIds] = useState();


    useEffect(() => {
        setFakerCount(1);
       
        client.service("courseDetails")
                        .find({ query: { $select: ["_id","courseName"] } })
                        .then((data) => {
                        setcourseNameIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = studentDetailsFakerFactory(props.user,fakerCount,courseNameIds);
