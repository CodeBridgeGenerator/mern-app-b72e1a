...
import errorsMenakaFakerFactory from "./errorsMenakaFakerFactory";
...
const ErrorsMenakaFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = errorsMenakaFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("errorsMenaka").create(elem));
...
export default connect(mapState, mapDispatch)(ErrorsMenakaFakerDialogComponent);
