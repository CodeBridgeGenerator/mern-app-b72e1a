  ...
  const [isHelpSidebarVisible, setHelpSidebarVisible] = useState(false);
    ...
    useEffect(() => {
        ...
        client
            ...
            .then((res) => {
                set_entity(res || {});
                const courseName = Array.isArray(res.courseName)
            ? res.courseName.map((elem) => ({ _id: elem._id, courseName: elem.courseName }))
            : res.courseName
                ? [{ _id: res.courseName._id, courseName: res.courseName.courseName }]
                : [];
  ...
  const copyPageLink = () => {
    ...
    return (
        ...
        <div className="col-12 flex flex-column align-items-center">
            ...
            <div className="col-12 md:col-6 lg:col-3 mb-10"><label className="text-sm text-gray-600">StuName</label><p className="m-0 ml-3" >{_entity?.stuName}</p></div>
...
<div className="col-12 md:col-6 lg:col-3 mb-10"><label className="text-sm text-gray-600">Address</label><p className="m-0 ml-3" >{_entity?.address}</p></div>
            <div className="col-12 md:col-6 lg:col-3 mb-10"><label className="text-sm text-gray-600">CourseName</label>
                    {courseName.map((elem) => (
                        <Link key={elem._id} to={`/courseDetails/${elem._id}`}>
                        <div>
                  {" "}
                            <p className="text-xl text-primary">{elem.courseName}</p>
                            </div>
                        </Link>
                    ))}</div>
