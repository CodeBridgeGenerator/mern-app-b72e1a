...
import departmentDetailsFakerFactory from "./departmentDetailsFakerFactory";
...
const DepartmentDetailsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = departmentDetailsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("departmentDetails").create(elem));
...
export default connect(mapState, mapDispatch)(DepartmentDetailsFakerDialogComponent);
