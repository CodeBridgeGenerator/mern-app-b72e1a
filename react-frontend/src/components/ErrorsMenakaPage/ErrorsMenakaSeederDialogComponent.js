...
import seederData from "./errorsMenaka.seeder.json";
...
const ErrorsMenakaSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("errorsMenaka").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(ErrorsMenakaSeederDialogComponent);
