...
const DepartmentDetailsDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate0 = (rowData, { rowIndex }) => <p >{rowData.depName}</p>
const pTemplate1 = (rowData, { rowIndex }) => <p >{rowData.HoD}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "departmentDetails"});
...
<Column field="depName" header="DepName" body={pTemplate0} filter={selectedFilterFields.includes("depName")} hidden={selectedHideFields?.includes("depName")}  sortable style={{ minWidth: "8rem" }} />
<Column field="HoD" header="HoD" body={pTemplate1} filter={selectedFilterFields.includes("HoD")} hidden={selectedHideFields?.includes("HoD")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload DepartmentDetails Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search DepartmentDetails" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default DepartmentDetailsDataTable;