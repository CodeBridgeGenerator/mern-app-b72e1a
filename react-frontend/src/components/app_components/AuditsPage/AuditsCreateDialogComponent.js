...
const AuditsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            serviceName: _entity?.serviceName,action: _entity?.action,method: _entity?.method,details: _entity?.details,createdBy: _entity?.createdBy,
    ...
    return (
        <Dialog header="Create Audits" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
