...
const StudentDetailsCreateDialogComponent = (props) => {
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [courseName], setError);
        }
        set_entity({...init});
    }, [props.show]);
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            stuName: _entity?.stuName,DOB: _entity?.DOB,courseName: _entity?.courseName?._id,address: _entity?.address,
