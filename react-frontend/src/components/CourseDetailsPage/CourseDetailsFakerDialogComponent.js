...
const CourseDetailsFakerDialogComponent = (props) => {
    ...
    const [depNameIds, setdepNameIds] = useState();


    useEffect(() => {
        setFakerCount(1);
       
        client.service("departmentDetails")
                        .find({ query: { $select: ["_id","depName"] } })
                        .then((data) => {
                        setdepNameIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = courseDetailsFakerFactory(props.user,fakerCount,depNameIds);
